...
function spawnMob(idx) {
    // 挑戰已結束或尚未建立挑戰資料時停止補怪，避免清場後再次重生。
    if (mapState.current === 'four_challenge' && !window.fourRun) {
        mapState.mobs[idx] = null;
        return;
    }
    // 自製世界王挑戰只允許中央格(1)生成一隻；其餘四格保持空白。
    if (mapState.current === 'four_challenge' && window.fourRun && fourRun.mode === 'world' && idx !== 1) {
        mapState.mobs[idx] = null;
        return;
    }
