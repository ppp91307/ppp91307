...
function saveWarehouse(w){
    ...
    let payload = JSON.stringify({ items: items, gold: (w && w.gold) || 0 });
    // 倉庫屬於共用且高價值資料：每次覆寫前保留三代可復原備份。
    // 備份保存的是底層原始字串（可能為 LZ1），避免重新編碼時再度損壞。
    try {
        let previous = _lsGet(key);
        if(previous != null){
            let bak1 = _lsGet(key + '_bak1'), bak2 = _lsGet(key + '_bak2');
            if(bak2 != null) _lsSet(key + '_bak3', bak2);
            if(bak1 != null) _lsSet(key + '_bak2', bak1);
            _lsSet(key + '_bak1', previous);
        }
    } catch(e){}
    if(!_lzSet(key, payload)) return false;
    // 不只相信 setItem 的回傳值；立即讀回並核對完整內容。
    try {
        let verify = _lzGet(key);
        if(verify == null) return false;
        let parsed = JSON.parse(verify);
        if(!parsed || !Array.isArray(parsed.items)) return false;
        if(JSON.stringify({ items: parsed.items, gold: parsed.gold || 0 }) !== payload) return false;
    } catch(e){ return false; }
    return true;
}

function _whWriteOrRollback(w, invBefore, goldBefore){
    if(saveWarehouse(w)) return true;
    if(Array.isArray(invBefore)) player.inv = invBefore;
    if(goldBefore !== undefined) player.gold = goldBefore;
    if(typeof logSys === 'function') logSys('<span class="text-red-400 font-bold">倉庫寫入失敗，物品已安全退回背包，請重新操作。若持續發生，請先匯出存檔。</span>');
    return false;
}
...
function whOneClickDeposit(){
    let w = loadWarehouse();
    let invBefore = player.inv.map(it => ({...it}));
    ...
    if(!_whWriteOrRollback(w, invBefore)) { renderTabs(true); updateUI(); renderWarehouseNPC(document.getElementById('interaction-content')); return; }
    saveGame(); renderTabs(true); updateUI();
...
function whGold(dir){
    ...
    if(!_whWriteOrRollback(w, null, goldBefore)) { updateUI(); renderWarehouseNPC(document.getElementById('interaction-content')); return; }
    saveGame(); updateUI();
    renderWarehouseNPC(document.getElementById('interaction-content'));
}
