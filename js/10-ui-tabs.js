...
window.getEquipmentDisplayMode = function () {
    try { return localStorage.getItem('equipmentDisplayMode') === 'text' ? 'text' : 'grid'; }
    catch (_) { return window._equipmentDisplayMode === 'text' ? 'text' : 'grid'; }
};
window.setEquipmentDisplayMode = function (mode) {
    mode = mode === 'text' ? 'text' : 'grid';
    window._equipmentDisplayMode = mode;
    try { localStorage.setItem('equipmentDisplayMode', mode); } catch (_) {}
    if (typeof setEquipmentPanelEmbedded === 'function') setEquipmentPanelEmbedded(false);
    renderTabs(true);
    if (mode === 'grid' && typeof setEquipmentPanelEmbedded === 'function') setEquipmentPanelEmbedded(true);
};
function _renderEquipTab() {
    let eDiv = document.getElementById('tab-equip'); eDiv.innerHTML = '';
    if (getEquipmentDisplayMode() === 'grid') {
        eDiv.innerHTML = `<div class="equipment-tab-launcher">
            ...
            <div class="equipment-tab-launcher-actions">
                ...
                <button class="btn" onclick="setEquipmentDisplayMode('text')">切換文字顯示</button>
            </div>
            <div class="equipment-tab-launcher-help">單擊裝備可開啟背包，雙擊已穿裝備可卸除；使用下方按鈕切換兩頁。</div>
        </div>`;
        if (typeof refreshEquipmentWindow === 'function') refreshEquipmentWindow();
        return;
    }
    eDiv.innerHTML = `<div class="equipment-text-view-toolbar"><strong>文字裝備清單</strong><button class="btn" onclick="setEquipmentDisplayMode('grid')">切換格子顯示</button></div>`;
    ...
    slots.forEach(s => {
        ...
        let isSherineActive = !!(eq && DB.items[eq.id] && DB.items[eq.id].sherineRemain && eq.seteff && player._sherineSetCnt && (player._sherineSetCnt[eq.seteff.slice(0, 2)] || 0) >= 2);
...
player.inv.forEach(i => {
    ...
    el.dataset.tipUid = i.uid;
    el.dataset.tipSrc = 'inv';
...
function buildItemDescHTML(item) {
    ...
    if (item.seteff) {
        ...
        desc = `<span class="c-sherine font-bold">✦ 席琳套裝效果：${_g}</span><br>${_lines}`
             + (desc ? `<br>${desc}` : '');
        if (!d.sherineRemain) desc += '<br><span class="text-amber-300 font-bold">⚠ 舊裝備詞綴只保留顯示，不再計入套裝件數。請到席琳神殿找菈克希絲免費拆成遺骸。</span>';
    }
    if(d.type === 'wpn') {
...
function openModal(item, isEq, slot) {
    ...
    if (item.fourRoll && item.fourRoll.length) {
        desc += `<div class="mt-3 pt-3 border-t border-cyan-700/60"><span class="text-cyan-300 font-bold">✨ 洗鍊效果</span>${item.fourRoll.map(x=>`<br><span class="text-yellow-300">・${x.n} +${x.v}</span>`).join('')}</div>`;
    }
...
function switchTab(t, btn) {
    ...
    if(typeof setEquipmentPanelEmbedded === 'function') {
        const showGridEquipment = t === 'equip' && (typeof getEquipmentDisplayMode !== 'function' || getEquipmentDisplayMode() === 'grid');
        setEquipmentPanelEmbedded(showGridEquipment);
    }
