...
let _lootMonsterName = '';   // 怪物擊殺掉落日誌的暫時來源；離開 killMob 後立即清空
...
function killMob(idx) {
    ...
    _lootMonsterName = mob.n || '';   // 掉落日誌來源：僅 killMob 結算期間標示怪物名稱，商店／任務獎勵不受影響
    try {
    ...
    if (mob._sherine) {
        ...
        _lootMonsterName = '';    // 結束本次擊殺掉落來源，避免後續購買／任務獎勵誤標怪物
    }
    ...
    // 自製挑戰系統原生擊殺掛點：只通知模組，不替換 killMob。
    if (typeof fourOnMobKilled === 'function') fourOnMobKilled(mob);
}
